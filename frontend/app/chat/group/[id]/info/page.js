...
export default function GroupInfoPage() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-black flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        ...
        <p className="text-red-500/60 text-xs tracking-widest font-bold"
          style={{fontFamily: "'Orbitron', sans-serif"}}>LOADING...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-black text-white"
      style={{fontFamily: "'Rajdhani', sans-serif", paddingTop: '44px'}}>
      ...
      {/* HERO BANNER */}
      <div className="relative h-52 overflow-hidden scan-line"
        style={{background: 'linear-gradient(135deg, #0a0014 0%, #000 40%, #00040a 100%)'}}>
        ...
        <button onClick={() => router.back()}
          className="absolute top-5 left-5 flex items-center gap-2 text-gray-400 hover:text-red-400 font-bold text-sm bg-black/60 px-3 py-2 rounded-lg backdrop-blur-sm border border-gray-800 transition">
          ← BACK
        </button>

        <button onClick={() => router.push(`/chat/group/${params.id}`)}
          className="absolute top-5 right-5 flex items-center gap-2 text-white font-black text-xs tracking-widest bg-red-600 hover:bg-red-500 px-4 py-2 rounded-lg transition"
      ...
      <div className="max-w-2xl mx-auto px-6">
        <div className="relative -mt-14 mb-6 flex items-end gap-5">
          ...
          <div className="pb-2 flex-1">
            <h1 className="font-black text-3xl tracking-wide leading-tight"
              style={{fontFamily: "'Orbitron', sans-serif", textShadow: '0 0 20px rgba(99,102,241,0.3)'}}>
            ...
            <div className="flex items-center gap-3 mt-1">
              ...
              {adminMember && (
                <span className="text-gray-600 text-xs font-bold">
                  · CREATED BY {adminMember.users?.name?.toUpperCase()}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="mb-4 rounded-2xl p-5 border"
          style={{background: 'linear-gradient(135deg, #0a0a0a, #0d0005)', borderColor: 'rgba(239,68,68,0.4)', boxShadow: '0 0 30px rgba(239,68,68,0.1)'}}>
          <div className="flex items-center justify-between mb-4">
            <p className="text-xs font-black tracking-widest text-red-500/70"
              style={{fontFamily: "'Orbitron', sans-serif"}}>⚡ GROUP CODE</p>
          ...
          <p className="text-xs text-gray-600 font-bold text-center mt-3">
        ...
        {group?.description ? (
          <div className="card mb-4">
            <p className="text-xs font-black tracking-widest text-red-500/70 mb-2"
              style={{fontFamily: "'Orbitron', sans-serif"}}>// ABOUT</p>
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          ...
          <div className="card text-center" style={{padding: '16px'}}>
            <div className="text-xs font-black text-gray-400 mt-1"
        ...
        {/* MEMBERS LIST */}
        <div className="card">
          <p className="text-xs font-black tracking-widest text-red-500/70 mb-4"
            style={{fontFamily: "'Orbitron', sans-serif"}}>// MEMBERS ({members.length})</p>
              ...
              <Link href={`/profile/${member.user_id}`} key={i}>
                <div className="member-row flex items-center gap-3 p-3 cursor-pointer transition">
                  ...
                  <div className="flex items-center gap-2">
                    {member.role === 'admin' && (
                      <span className="text-xs bg-yellow-500/10 text-yellow-400 border border-yellow-500/30 px-2 py-0.5 rounded font-black">
                        👑 ADMIN
                      </span>
                    )}
